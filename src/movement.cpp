...
uint32_t MoveEvent::AddItemField(Item* item, Item* tileItem, const Position& pos)
{
	if(MagicField* field = item->getMagicField()){
		...
		for(CreatureVector::iterator cit = tile->creatures.begin();
			cit != tile->creatures.end(); ++cit)
		{
...
uint32_t MoveEvent::fireStepEvent(Creature* creature, Item* item,
	const Position& fromPos, const Position& toPos)
{
	if(m_scripted){
...
uint32_t MoveEvent::executeStep(Creature* creature, Item* item, 
	const Position& fromPos, const Position& toPos)
{
...
ReturnValue MoveEvent::canPlayerWearEquip(Player* player, slots_t slot)
{
	...
	if(player->isItemAbilityEnabled(slot) ||
		player->hasFlag(PlayerFlag_IgnoreWeaponCheck) ||
		getWieldInfo() == 0){
		return RET_NOERROR;
	}
