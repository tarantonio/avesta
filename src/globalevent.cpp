...
bool GlobalEvents::registerEvent(Event* event, xmlNodePtr)
{
	...
	if(globalEvent->getEventType() == GLOBALEVENT_TIMER)
	{
		...
		if(it == timerMap.end())
		{
			...
			if(timerEventId == 0)
			{
				timerEventId = Scheduler::getScheduler().addEvent(
					createSchedulerTask(TIMER_INTERVAL,
	...
	else // think event
	{
		...
		if(it == thinkMap.end())
		{
			...
			if(thinkEventId == 0)
			{
				thinkEventId = Scheduler::getScheduler().addEvent(
					createSchedulerTask(SCHEDULER_MINTICKS,
